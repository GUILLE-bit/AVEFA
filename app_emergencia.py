...
def fetch_bytes(url: str) -> bytes:
    # Descargar bytes desde una URL (GitHub raw u otra)
    # Sin dependencias externas (usa urllib). Agrega user-agent para evitar bloqueos.
...
class PracticalANNModel:
    def __init__(self, IW, bias_IW, LW, bias_out):
        ...
        # Orden de entrada alineado a tus pesos: [Julian_days, TMIN, TMAX, Prec]
...
# ====================== UI ======================
st.title("Predicción de Emergencia Agrícola con ANN (pesos .npy desde GitHub)")

st.sidebar.header("Pesos .npy (URLs RAW de GitHub)")
base_raw = st.sidebar.text_input(
    "Base RAW (opcional)",
    value="https://raw.githubusercontent.com/USER/REPO/BRANCH/weights",
    help="Si completás esto, abajo podés usar solo los nombres de archivo."
)
fname_IW = st.sidebar.text_input("IW.npy", value="IW.npy")
fname_bIW = st.sidebar.text_input("bias_IW.npy", value="bias_IW.npy")
fname_LW = st.sidebar.text_input("LW.npy", value="LW.npy")
fname_bout = st.sidebar.text_input("bias_out.npy", value="bias_out.npy")

def _join(base, name):
    return base.rstrip("/") + "/" + name.lstrip("/")

url_IW = _join(base_raw, fname_IW) if base_raw else st.sidebar.text_input("URL IW.npy")
url_bIW = _join(base_raw, fname_bIW) if base_raw else st.sidebar.text_input("URL bias_IW.npy")
url_LW = _join(base_raw, fname_LW) if base_raw else st.sidebar.text_input("URL LW.npy")
url_bout = _join(base_raw, fname_bout) if base_raw else st.sidebar.text_input("URL bias_out.npy")

colA, colB = st.sidebar.columns(2)
with colA:
    if st.button("Probar descarga de pesos"):
        try:
            _ = load_npy_from_url(url_IW)
            _ = load_npy_from_url(url_bIW)
            _ = load_npy_from_url(url_LW)
            _ = load_npy_from_url(url_bout)
            st.success("Descarga OK ✅")
        except Exception as e:
            st.error(f"Falla al descargar: {e}")
...
if st.sidebar.button("Forzar recarga de cache"):
    st.cache_data.clear()
...
if dfs:
    for nombre, df in dfs:
        ...
        st.download_button(f"Descargar resultados (rango) - {nombre}", tabla.to_csv(index=False).encode("utf-8"), f"{nombre}_resultados_rango.csv", "text/csv")
